"""
**Custom general datatypes for use in PHDP.**
...
class PHDPBase:
    """
    Defines a base class with common behaviors for all PHDP objects.

    Example of representation strings and printing via ``__repr__`` and ``__str__`` methods:

    ::

        class ExamplePHDPClass(PHDPBase):
        def __init__(self):
            self.first_attribute = 0
            self.second_attribute = 1

        >>>example_object = ExamplePHDPClass()
        >>>example_object
        <PHDP2 ExamplePHDPClass object at 0x7f91d03975e0>
    ...
    def __repr__(self):
        ...
        """
        return "<PHDP2 %s object at 0x%x>" % (type(self).__name__,  id(self))
...
class PHDPEnum(PHDPBase):
    ...
    ::
        ...
        # define an PHDPEnum
        reg_classes = PHDPEnum(['car', 'truck'])
    ...
    def __init__(self, enum_list):
        """
        Create PHDPEnum object from list of values.
...
if __name__ == "__main__":
    ...
    try:
        test_enum = PHDPEnum(['foo', 'bar', 'space force!'])
